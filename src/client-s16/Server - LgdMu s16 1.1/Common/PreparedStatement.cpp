...
void PreparedStatement::BindParameters()
{
    ...
    if (i < m_stmt->m_paramCount)
        sLog->outWarn(LOG_SQL, "[WARNING]: BindParameters() for statement %u did not bind all allocated parameters", m_index);
    #endif
}
...
void PreparedStatement::setStringBase64(const uint8 index, const std::string& value)
{
	this->setString(index, base64::encode((uint8*)value.c_str(), value.size()));
}
...
{
    sLog->outError(LOG_SQL, "Attempted to bind parameter %u%s on a PreparedStatement %u (statement has only %u parameters)", uint32(index) + 1, (index == 1 ? "st" : (index == 2 ? "nd" : (index == 3 ? "rd" : "nd"))), stmtIndex, paramCount);
    return false;
}
...
{
    WPAssert(index < m_paramCount || ParamenterIndexAssertFail(m_stmt->m_index, index, m_paramCount));

    if (m_paramsSet[index])
        sLog->outWarn(LOG_SQL, "[WARNING] Prepared Statement (id: %u) trying to bind value on already bound index (%u).", m_stmt->m_index, index);
    return true;
}
