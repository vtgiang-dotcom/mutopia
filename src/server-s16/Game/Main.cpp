...
{
	sLog->outInfo(LOG_DEFAULT, "Running MainApplication::Run()...");
	...
	sLog->outInfo(LOG_DEFAULT, "Loading GameServer settings...");
	sGameServer->LoadCommonSettings();
	sGameServer->LoadCashShopSettings();

	sLog->outInfo(LOG_DEFAULT, "Loading Message and Formula...");
	sMessage->Init();

	sFormulaMgr->Load();

	sLog->outInfo(LOG_DEFAULT, "Loading World...");
	...
	sLog->outInfo(LOG_DEFAULT, "Loading Object and Items...");
	sObjectMgr->Init();
...
void MainApplication::Update(uint32 diff)
{
	...
	{
		sLog->outInfo("root", "[UPDATE TICK %u] Begin diff=%u", tickCount, diff);
	}
	...
	if (tickCount <= 5 || (tickCount % 100) == 0)
	{
		sLog->outInfo("root", "[UPDATE TICK %u] End successfully", tickCount);
	}
}