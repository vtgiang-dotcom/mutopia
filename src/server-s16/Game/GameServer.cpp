...
void GameServer::LoadCommonSettings()
{
	...
	std::string temp_client_version = sMain->GetString("GameServer.Version", "1.04.05");
	if ( temp_client_version.empty() )
	{
		temp_client_version = "1.04.05";
	}
	...
	if ( tokens.size() >= 3 && strlen(tokens[0]) >= 1 && strlen(tokens[1]) >= 2 && strlen(tokens[2]) >= 2 )
	{
	...
	{
		this->SetGameServerVersion(0, '1');
		this->SetGameServerVersion(1, '0');
		this->SetGameServerVersion(2, '4');
		this->SetGameServerVersion(3, '0');
		this->SetGameServerVersion(4, '5');
	}
