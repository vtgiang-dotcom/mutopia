...
{
	sLog->outInfo(LOG_DEFAULT, "SignalHandler triggered: error=%d (%s), signal=%d", error.value(), error.message().c_str(), signalNumber);
    if (!error)
...
void AppConsole::UpdateLoop()
{
	...
	while (!AppConsole::IsStopped())
    {
		...
		{
			sLog->outError(LOG_DEFAULT, "FATAL EXCEPTION in Update(%u): %s", diff, e.what());
			break;
		}
		catch (...)
		{
			sLog->outError(LOG_DEFAULT, "FATAL UNKNOWN EXCEPTION in Update(%u)", diff);
			break;
		}
