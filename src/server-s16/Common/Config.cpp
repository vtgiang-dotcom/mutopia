...
bool Config::LoadInitial(std::string const& file, std::string& error, std::string const& section)
{
    ...
    if (GetFullPathNameA(file.c_str(), MAX_PATH, fullPath, NULL) > 0)
    {
        _filename = fullPath;
    }
    else
    ...
    {
        ptree fullTree;
        ini_parser::read_ini(_filename, fullTree);
...
std::string Config::GetStringDefault(std::string const& name, const std::string& def)
{
    ...
    GetPrivateProfileStringA(_section.c_str(), name.c_str(), def.c_str(), returned_string, 1000, _filename.c_str());

	std::string value = returned_string;
	value.erase(std::remove(value.begin(), value.end(), '"'), value.end());
...
#else
	int result = GetPrivateProfileIntA(_section.c_str(), name.c_str(), def, _filename.c_str());
...
{
	WritePrivateProfileStringA(_section.c_str(), name.c_str(), value.c_str(), _filename.c_str());
}
	
void Config::WriteInt(std::string const& name, int32 value)
{
	WritePrivateProfileStringA(_section.c_str(), name.c_str(), std::to_string(value).c_str(), _filename.c_str());
}