    ...
    public async ValueTask ShowAsync(Player receiver, QuestRewardType reward, int value, AttributeDefinition? attributeReward)
    {
        ...
        switch (reward)
        {
            ...
            case QuestRewardType.CharacterEvolutionFirstToSecond:
                await this._player.Connection.SendLegacyQuestRewardAsync(receiverId, LegacyQuestReward.QuestRewardType.CharacterEvolutionFirstToSecond, (byte)this._player.SelectedCharacter!.CharacterClass!.Number).ConfigureAwait(false);
                break;
            case QuestRewardType.CharacterEvolutionSecondToThird:
                await this._player.Connection.SendLegacyQuestRewardAsync(receiverId, LegacyQuestReward.QuestRewardType.CharacterEvolutionSecondToThird, (byte)this._player.SelectedCharacter!.CharacterClass!.Number).ConfigureAwait(false);
                break;
