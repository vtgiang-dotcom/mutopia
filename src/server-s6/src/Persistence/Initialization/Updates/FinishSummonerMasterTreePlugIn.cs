...
public class FinishSummonerMasterTreePlugIn : UpdatePlugInBase
{
    ...
    protected override async ValueTask ApplyAsync(IContext context, GameConfiguration gameConfiguration)
    {
        ...
        if (berserkerEffect.Duration != null)
        {
            berserkerEffect.Duration.MaximumValue = 180;
        }

        if (berserkerEffect.PowerUpDefinitions.FirstOrDefault(pud => pud.TargetAttribute == Stats.BerserkerManaMultiplier) is PowerUpDefinition berserkerManaPowerUp && berserkerManaPowerUp.Boost != null)
        {
