...
public static class AttackableExtensions
{
    ...
    /// </summary>
    public static bool IsSummonedMonster(this IAttackable attackable) => attackable is Monster { SummonedBy: not null };
    ...
    public static bool CheckSkillTargetRestrictions(this IAttackable target, Player player, Skill skill)
    {
        ...
        else if (skill.TargetRestriction == SkillTargetRestriction.Player && target is not Player && target is Monster { SummonedBy: null })
        {
            ...
            result = false;
                 ...
                 && MUnique.OpenMU.GameLogic.PlugIns.ChatCommands.GuildWarEventChatCommandPlugIn.IsInSameFaction(player, targetPlayer)
                 && skill.SkillType is not SkillType.Buff and not SkillType.Regeneration)
        {
            result = false;
        }
        else
