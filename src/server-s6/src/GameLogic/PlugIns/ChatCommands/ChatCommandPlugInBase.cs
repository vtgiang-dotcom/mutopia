    ...
    where T : ArgumentsBase, new()
{
    ...
    protected WarpInfo? GetWarpInfo(Player player, string map)
    {
        ...
        if (ushort.TryParse(map, out var mapId))
        {
            return warpList.FirstOrDefault(info => info.Gate?.Map?.Number == mapId);
        }

        var normalizedMap = map.Replace(" ", "").Replace("_", "");
        return warpList.FirstOrDefault(info => map.Equals(info.Name.GetTranslationAsSpan(player.Culture).ToString(), StringComparison.CurrentCultureIgnoreCase))
            ?? warpList.FirstOrDefault(info => normalizedMap.Equals(info.Name.GetTranslationAsSpan(player.Culture).ToString().Replace(" ", "").Replace("_", ""), StringComparison.CurrentCultureIgnoreCase))
            ?? (normalizedMap.EndsWith("1", StringComparison.OrdinalIgnoreCase)
                ? warpList.FirstOrDefault(info => normalizedMap[..^1].Equals(info.Name.GetTranslationAsSpan(player.Culture).ToString().Replace(" ", "").Replace("_", ""), StringComparison.CurrentCultureIgnoreCase))
                : null);
    }
