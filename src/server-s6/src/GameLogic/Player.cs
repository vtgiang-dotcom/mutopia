...
public class Player : AsyncDisposable, IBucketMapObserver, IAttackable, IAttacker, ITrader, IPartyMember, IRotatable, IHasBucketInformation, ISupportWalk, IMovable, ILoggerOwner<Player>
{
    ...
    public virtual async ValueTask OnPickedUpItemAsync(ILocateable item)
    {
    ...
    /// <summary>
    /// Will be called when an item has been consumed by a player.
    /// </summary>
    /// <param name="item">The item that the player has consumed.</param>
    public virtual void OnItemConsumed(Item item)
    {
    }

    /// <summary>
    /// Will be called when an item has been dropped by a player.
    /// </summary>
    /// <param name="item">The item that the player has dropped.</param>
    ...
    public virtual bool TryRemoveMoney(int value)
    {
        if (this.Money < value)
        {
            return false;
    ...
    public int CalculateExpAfterKill(IAttackable killedObject)
    {
        ...
        experience *= this.GetVipExpMultiplier();
    ...
    /// <summary>
    /// Gets the VIP EXP multiplier for this player based on their account VIP status.
    /// </summary>
    public float GetVipExpMultiplier()
    {
        if (this.Account is { } account && account.VipTier > 0 && (account.VipExpiry is null || account.VipExpiry > DateTime.UtcNow))
        {
            return account.VipTier switch
            {
                1 => 1.20f, // Silver: +20%
                2 => 1.50f, // Gold: +50%
                3 => 2.00f, // Platinum: +100%
                _ => 1.00f,
            };
        }
    ...
    internal async ValueTask AfterKilledPlayerAsync(Player killedPlayer)
    {
        ...
        if (PlugIns.ChatCommands.GuildWarEventChatCommandPlugIn.IsWarRunning
            || (this is Offline.OfflinePlayer && killedPlayer is Offline.OfflinePlayer))
        {
            return;
        }
    ...
    protected virtual async ValueTask AddExperienceCoreAsync(int experience, IAttackable? killedObject)
    {
