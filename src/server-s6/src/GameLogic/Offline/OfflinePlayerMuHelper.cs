...
public sealed class OfflinePlayerMuHelper : AsyncDisposable
{
    ...
    private async ValueTask TickAsync(CancellationToken cancellationToken)
    {
        ...
        {
            // Non-defender bots freeze their AI completely so they don't wander into the GM's battlefield
            if (!PlugIns.ChatCommands.GuildWarEventChatCommandPlugIn.IsDefenderBot(this._player))
            {
                return;
            }

            // Defender bots stand at attention in formation during countdown
            if (!PlugIns.ChatCommands.GuildWarEventChatCommandPlugIn.IsCombatStarted)
            {
                return;
            }
        }
    ...
    private async ValueTask<bool> HandleDeathAsync()
    {
        if (this._isDead)
        {
            ...
            if (this._player.RespawnAndContinue && !PlugIns.ChatCommands.GuildWarEventChatCommandPlugIn.IsWarRunning)
            {
