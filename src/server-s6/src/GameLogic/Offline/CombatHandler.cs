...
public sealed class CombatHandler
{
    ...
    /// </summary>
    /// <summary>
    /// See <see cref="IsSafeTarget"/>: how far above the bot's own level a monster may still be counted
    /// as huntable. The game's weakest monster is level 2 (see the Lorencia spider) while a fresh bot
    /// starts at level 1 (see <see cref="BotGenerator"/>), so a strict "monster level must not exceed the
    /// bot's level" left every level-1 bot with no valid target at all - it could never level. Players
    /// routinely fight monsters 1-2 levels above them, so the slack mirrors that; the real survivability
    /// and offense checks below stay in charge of what is actually safe.
    /// </summary>
    private const int SafeTargetLevelSlack = 2;
    ...
    private int _comboRotationIndex;
    private int _warAttackTicks;
    ...
    /// exceed the bot's own by more than <see cref="SafeTargetLevelSlack"/> (on reset servers: its
    /// reset-aware effective level, see <see cref="BotResetHandler.GetEffectiveLevel"/>).
    ...
    public static bool IsSafeTarget(Player player, MonsterDefinition monster)
    {
        ...
        // being locked out of the maps it just hunted on. A monster may exceed that level by
        // SafeTargetLevelSlack (the weakest monster is level 2, a fresh bot is level 1); the damage and
        // offense checks below keep the actual safety, this is only a coarse gate.
        if (monsterLevel > BotResetHandler.GetEffectiveLevel(player) + SafeTargetLevelSlack)
        {
            return false;
        }
    ...
    public async ValueTask PerformAttackAsync()
    {
        ...
        if (this.IsBot && !MUnique.OpenMU.GameLogic.PlugIns.ChatCommands.GuildWarEventChatCommandPlugIn.IsWarRunning)
        {
    ...
    private void RefreshTarget()
    {
        ...
        {
            this._warAttackTicks++;
            if (this._warAttackTicks % 3 == 0 || this._currentTarget is null || !this.IsTargetStillValid(this._currentTarget))
            {
                var warTargets = this.GetAttackableTargetsInHuntingRange().ToList();
                if (warTargets.Count > 0)
                {
                    var sortedWarTargets = warTargets
                        .OrderBy(t => t.GetDistanceTo(this._player))
                        .ThenBy(t => (t as Player)?.Attributes?[Stats.CurrentHealth] ?? 99999)
                        .Take(3)
                        .ToList();
                    this._currentTarget = sortedWarTargets.SelectRandom();
                    this._nearbyMonsterCount = warTargets.Count;
                    this._targetsAroundCurrent = CountPackAround(this._currentTarget, warTargets);
                    return;
                }
            }
        }
    ...
    private IEnumerable<IAttackable> GetAttackableTargetsInHuntingRange()
    {
        ...
        var isCombatActive = MUnique.OpenMU.GameLogic.PlugIns.ChatCommands.GuildWarEventChatCommandPlugIn.IsCombatStarted;
        var isWarEvent = MUnique.OpenMU.GameLogic.PlugIns.ChatCommands.GuildWarEventChatCommandPlugIn.IsWarRunning;
        var searchRange = isCombatActive ? (byte)30 : this.HuntingRange;
        return map.GetAttackablesInRange(this.OriginPosition, searchRange)
            .Where(attackable => attackable switch
            {
                Monster monster => this.IsMonsterAttackable(monster),
                Player player => (freeForAll || (isCombatActive && !isWarEvent)) && this.IsEventRivalAttackable(player, isCombatActive),
                _ => false,
            });
    }

    private bool IsEventRivalAttackable(Player target, bool isCombatActive = false)
    {
        return !ReferenceEquals(target, this._player)
               && target.IsAlive
               && (isCombatActive || !target.IsAtSafezone())
    ...
    private bool IsTargetStillValid(IAttackable target)
    {
        ...
        var isCombatActive = MUnique.OpenMU.GameLogic.PlugIns.ChatCommands.GuildWarEventChatCommandPlugIn.IsCombatStarted;
        var maxRange = isCombatActive ? (byte)30 : this.HuntingRange;

        return target.IsAlive
               && !target.IsAtSafezone()
               && !target.IsTeleporting
               && target.IsInRange(this.OriginPosition, maxRange);
    }
    ...
    private bool IsWithinSafeHuntLevel(Monster monster)
    {
        ...
        if (this._player.CurrentMiniGame is not null || MUnique.OpenMU.GameLogic.PlugIns.ChatCommands.GuildWarEventChatCommandPlugIn.IsWarRunning)
        {
            // Inside a mini game or war defense event, fight all oncoming monsters and bosses
            return true;
        }
    ...
    private SkillEntry? SelectBestAffordableSkill()
    {
        ...
        if (MUnique.OpenMU.GameLogic.PlugIns.ChatCommands.GuildWarEventChatCommandPlugIn.IsWarRunning && candidates.Count > 0)
        {
            // Prefer AoE and high-range skills during Horde defense so casters don't pick 1-range weapon pokes
            var rangedCandidates = candidates.Where(c => (c.Entry.Skill?.Range ?? 0) >= 3).ToList();
            var pool = rangedCandidates.Count > 0 ? rangedCandidates : candidates;
            this._comboRotationIndex = (this._comboRotationIndex + 1) % pool.Count;
            return this._tickBestSkill = pool[this._comboRotationIndex].Entry;
        }
