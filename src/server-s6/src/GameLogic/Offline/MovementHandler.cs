...
public sealed class MovementHandler
{
    ...
    public async ValueTask<bool> RegroupAsync()
    {
        if (MUnique.OpenMU.GameLogic.PlugIns.ChatCommands.GuildWarEventChatCommandPlugIn.IsWarRunning)
        {
            ...
            // In war defense mode, keep defenders tethered tightly to their designated defense post
            if (this._player.Position.EuclideanDistanceTo(this.OriginPosition) > 2)
            {
    ...
    public async ValueTask<bool> MoveCloserToTargetAsync(IAttackable target, byte range)
    {
        if (MUnique.OpenMU.GameLogic.PlugIns.ChatCommands.GuildWarEventChatCommandPlugIn.IsWarRunning)
        {
            ...
            if (this._player.CurrentMap is { } warMap)
            {
                var walkTarget = GetApproachPoint(warMap, this._player.Position, target.Position, range);
                if (walkTarget.EuclideanDistanceTo(this.OriginPosition) <= 3)
                {
