...
public sealed class ItemPickupHandler
{
    ...
    /// <summary>Lower bound of the jittered interval between two bot pickup scans.</summary>
    private static readonly TimeSpan MinPickupScanInterval = TimeSpan.FromMilliseconds(400);

    /// <summary>Upper bound of the jittered interval between two bot pickup scans.</summary>
    private static readonly TimeSpan MaxPickupScanInterval = TimeSpan.FromMilliseconds(1400);

    private static readonly PickupItemAction PickupAction = new();

    private DateTime _nextPickupScanUtc = DateTime.MinValue;
    ...
    public async ValueTask PickupItemsAsync()
    {
        ...
        // A bot which vacuums up every drop the same 500ms tick it lands looks robotic; a player wanders
        // over and stoops after a moment. Rate-limit the scan and roll a fresh jitter each time, so two
        // bots side by side do not stoop in lockstep. Humans still get the plain (every-tick) behavior.
        var now = DateTime.UtcNow;
        if (this._player.Account?.IsBot == true)
        {
            if (now < this._nextPickupScanUtc)
            {
                return;
            }

            this._nextPickupScanUtc = now + MinPickupScanInterval
                + TimeSpan.FromMilliseconds(Rand.NextInt(0, (int)(MaxPickupScanInterval - MinPickupScanInterval).TotalMilliseconds + 1));
        }
