...
public class BotFeaturePlugIn : IFeaturePlugIn, IPeriodicTaskPlugIn, ISupportCustomConfiguration<BotConfiguration>, ISupportDefaultCustomConfiguration
{
    ...
    /// <summary>
    /// Gets all currently active BotPlayer instances across all game servers.
    /// </summary>
    /// <returns>A list of active <see cref="BotPlayer"/> objects.</returns>
    public IReadOnlyList<BotPlayer> GetAllActiveBots()
    {
    ...
    /// <summary>
    /// Kills a specific bot by account login name across all game servers.
    /// </summary>
    /// <param name="loginName">The login name of the bot account.</param>
    /// <returns><c>true</c> if the bot was found and stopped; <c>false</c> otherwise.</returns>
    public async ValueTask<bool> KillBotAsync(string loginName)
    {
        foreach (var (_, state) in this._states)
        {
            if (await state.Manager.StopBotByLoginAsync(loginName).ConfigureAwait(false))
            {
                return true;
            }
        }
    ...
    public void ForceStart()
    {
        ...
        {
            // Reset a finished startup pass back to NotStarted, so the next tick re-runs the
            // population spawn with the current configuration (e.g. after the admin dashboard raised
            // NumberOfAccounts). Bumping NextRunUtc alone is a no-op once the pass is done, because
            // ExecuteTaskAsync routes a Done state straight to maintenance and never reaches the
            // NextRunUtc check. An in-progress pass is left alone - it already runs and must not be
            // started a second time concurrently.
            Interlocked.CompareExchange(ref state.StartupState, (int)StartupPhase.NotStarted, (int)StartupPhase.Done);
