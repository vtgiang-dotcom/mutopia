...
{
    /// <summary>
    /// Default margin for map warp hysteresis (overridden per bot personality).
    /// </summary>
    private const int DefaultWarpImprovementMargin = 8;
    ...
    /// don't come back together. See <see cref="GetNextShoppingCheckUtc"/>.
    /// </summary>
    ...
    /// <summary>
    /// Lower bound of the interval between two idle wander steps on a barren hunting ground: instead of
    /// standing stock-still while the empty-ground grace elapses, the bot occasionally shuffles a tile
    /// or two like a player scanning for a spawn.
    /// </summary>
    private static readonly TimeSpan MinWanderInterval = TimeSpan.FromSeconds(4);

    /// <summary>Upper bound of the idle wander interval, see <see cref="MinWanderInterval"/>.</summary>
    private static readonly TimeSpan MaxWanderInterval = TimeSpan.FromSeconds(9);

    /// <summary>
    /// Lower bound of the time a bot idles (stands still, doing nothing) at a safezone. A real player
    /// pauses in town; a bot which never does is a giveaway. Personality scales the bounds (see
    /// <see cref="GetAfkBounds"/>).
    /// </summary>
    private static readonly TimeSpan MinAfkDuration = TimeSpan.FromSeconds(20);

    /// <summary>Upper bound of the safezone idle, see <see cref="MinAfkDuration"/>.</summary>
    private static readonly TimeSpan MaxAfkDuration = TimeSpan.FromMinutes(2);

    /// <summary>
    /// Minimum hunting time before a bot may take its next safezone AFK break, so a freshly spawned bot
    /// does not idle right at the gate it just walked out of.
    /// </summary>
    private static readonly TimeSpan AfkCooldown = TimeSpan.FromMinutes(30);
    ...
    private readonly int _warpImprovementMargin;
    private readonly double _shoppingCooldownMult;
    ...
    /// <summary>
    /// When this bot looks at its supplies again.
    /// </summary>
    private DateTime _nextShoppingCheckUtc;
    private DateTime? _resetDueAtUtc;
    private short _leaderMapNumber;
    private DateTime _leaderOnMapSinceUtc = DateTime.MinValue;

    private DateTime _nextWanderUtc = DateTime.MinValue;
    private DateTime? _afkUntilUtc;
    private DateTime _afkEligibleAtUtc = DateTime.MinValue;
    ...
    {
        this._player = player;
        this._personality = (player.MuHelperSettings as BotPersonalitySettings)?.Personality
            ?? BotPersonalityResolver.Resolve(player.Name);
        this._warpImprovementMargin = this._personality switch
        {
            BotPersonality.Warrior => 4,
            BotPersonality.Reckless => 2,
            BotPersonality.Guardian => 12,
            BotPersonality.Loner => 10,
            _ => DefaultWarpImprovementMargin,
        };
        this._shoppingCooldownMult = this._personality switch
        {
            BotPersonality.Greedy => 0.5,
            BotPersonality.Guardian => 0.8,
            BotPersonality.Reckless => 1.5,
            _ => 1.0,
        };
        this._nextShoppingCheckUtc = DateTime.UtcNow + (ShoppingCooldown * this._shoppingCooldownMult * Rand.NextDouble());
    }
    ...
    private DateTime GetNextShoppingCheckUtc()
    {
        var spread = 1 + (((Rand.NextDouble() * 2) - 1) * ShoppingSpread);
        return DateTime.UtcNow + (ShoppingCooldown * this._shoppingCooldownMult * spread);
    }

    private async Task SafeEvaluateAsync(CancellationToken cancellationToken)
    {
        if (MUnique.OpenMU.GameLogic.PlugIns.ChatCommands.GuildWarEventChatCommandPlugIn.IsWarRunning)
        {
            return;
        }
    ...
    private async ValueTask EvaluateAsync(CancellationToken cancellationToken)
    {
        ...
        // Occasional spontaneous speech (greeting a new face, idle town chatter) so the world is not mute
        // around players. Rate-limited and personality-aware; the Loner stays quiet (see BotChatHandler).
        BotChatHandler.ConsiderProactiveChat(this._player, inSafezone);
        ...
        // A human pauses in town; a bot which never does is an obvious giveaway. In the safezone, after a
        // decent stretch of hunting, the bot occasionally stops and idles for a short while before heading
        // back out. The AFK is deliberate, so it is measured separately from the stuck watchdog above and
        // simply short-circuits the routine for its duration - no walk is issued, no ground is re-picked.
        if (inSafezone && this.TryTakeAfkBreak())
        {
            return;
        }
        ...
        if (!inSafezone)
        {
            ...
            {
                // While waiting out the grace, occasionally shuffle a tile like a player scanning for a
                // spawn - a bot frozen on the spot next to an empty field is a giveaway. The step is a
                // single-tile walk to a neighboring walkable tile, jittered so neighboring bots don't step
                // in lockstep.
                await this.TryIdleWanderAsync(map).ConfigureAwait(false);
                return;
            }
        }
        ...
        if (this._shoppingTarget is { } target)
        {
            if (this._player.GetDistanceTo(target) > MerchantTalkRange)
            {
                if (!await this.TravelTowardAsync(map, target, cancellationToken).ConfigureAwait(false))
                {
                    ...
                    this._nextShoppingCheckUtc = this.GetNextShoppingCheckUtc();
                }
    ...
    private bool TryPickBetterMapCore(bool escape, int minimumMonsterLevel, out WarpInfo warp, out GameMapDefinition mapDefinition, out int monsterLevel)
    {
        ...
        var current = this._player.CurrentMap?.Definition;
        var threshold = escape ? 1 : (current is null ? 0 : this.BestSafeLevel(current, minimumMonsterLevel)) + this._warpImprovementMargin;
    ...
    /// <summary>
    /// Rolls whether the bot takes its next safezone AFK break, and if so, arms the idle window. Returns
    /// true while the idle window is active, so the routine short-circuits and the bot stands still.
    /// </summary>
    private bool TryTakeAfkBreak()
    {
        var now = DateTime.UtcNow;

        if (this._afkUntilUtc is { } afkUntil)
        {
            if (now < afkUntil)
            {
                return true;
            ...
            this._afkUntilUtc = null;
            return false;
        }

        // Only after a solid stretch of hunting since spawn or the last break, and only for a personality-
        // scaled chance per visit - a Reckless bot barely pauses, a Guardian lingers more.
        if (now < this._afkEligibleAtUtc)
        {
            return false;
        }

        var (minAfk, maxAfk, chance) = this.GetAfkBounds();
        if (!Rand.NextRandomBool(chance))
        {
            return false;
        }

        this._afkUntilUtc = now + minAfk + TimeSpan.FromSeconds(Rand.NextInt(0, (int)(maxAfk - minAfk).TotalSeconds + 1));
        this._afkEligibleAtUtc = now + AfkCooldown;
        return true;
    }

    /// <summary>
    /// Per-personality bounds and probability of a safezone AFK break, returned as
    /// (minimum duration, maximum duration, chance in percent per safezone visit).
    /// </summary>
    private (TimeSpan Min, TimeSpan Max, int ChancePercent) GetAfkBounds()
    {
        return this._personality switch
        {
            BotPersonality.Reckless => (MinAfkDuration, MinAfkDuration, 10),
            BotPersonality.Warrior => (MinAfkDuration, TimeSpan.FromSeconds(40), 15),
            BotPersonality.Greedy => (TimeSpan.FromSeconds(30), TimeSpan.FromMinutes(1), 20),
            BotPersonality.Loner => (TimeSpan.FromSeconds(30), TimeSpan.FromMinutes(1), 25),
            BotPersonality.Guardian => (MinAfkDuration, MaxAfkDuration, 30),
            _ => (MinAfkDuration, TimeSpan.FromMinutes(1), 20),
        };
    }

    /// <summary>
    /// On a barren hunting ground, occasionally takes a single-tile step to a neighboring walkable tile,
    /// so the bot shuffles around like a player scanning for a spawn instead of standing frozen.
    /// </summary>
    private async ValueTask TryIdleWanderAsync(GameMap map)
    {
        var now = DateTime.UtcNow;
        if (now < this._nextWanderUtc)
        {
            return;
        }

        this._nextWanderUtc = now + MinWanderInterval
            + TimeSpan.FromSeconds(Rand.NextInt(0, (int)(MaxWanderInterval - MinWanderInterval).TotalSeconds + 1));
        ...
        // Shuffle the directions so the step does not always bias the same way.
        ...
        {
            var candidate = position.CalculateTargetPoint(direction);
            if (candidate.X >= map.Terrain.WalkMap.GetLength(0) || candidate.Y >= map.Terrain.WalkMap.GetLength(1))
            {
                continue;
            }

            if (!map.Terrain.WalkMap[candidate.X, candidate.Y] || map.Terrain.SafezoneMap[candidate.X, candidate.Y])
            {
                continue;
            }

            var steps = new[] { new WalkingStep(position, candidate, position.GetDirectionTo(candidate)) };
            await this._player.WalkToAsync(candidate, steps).ConfigureAwait(false);
            return;
        }
    }
