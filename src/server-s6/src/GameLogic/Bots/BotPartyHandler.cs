...
/// <see cref="MuHelper.IMuHelperSettings.AutoAcceptAnyone"/>): the invitation is accepted after a short
...
internal static class BotPartyHandler
{
    ...
    internal static async ValueTask<bool> TryScheduleAcceptAsync(Player receiver, Player requester, TimeSpan? acceptDelay = null)
    {
        if (receiver is not OfflinePlayer bot
            ...
            || bot.MuHelperSettings?.AutoAcceptAnyone != true
        ...
        var acceptAt = acceptDelay.HasValue
            ? DateTime.UtcNow.Add(acceptDelay.Value).AddSeconds(-1)
        ...
        bot.PendingPartyInvite = new PendingPartyInvite(requester, acceptAt);
    ...
    {
        if (bot.PendingPartyInvite is { } invite && DateTime.UtcNow.AddMilliseconds(10) >= invite.AcceptAtUtc)
        {
