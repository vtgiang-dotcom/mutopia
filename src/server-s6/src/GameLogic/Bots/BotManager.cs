...
public sealed class BotManager
{
    ...
    public async ValueTask<bool> SpawnBotAsync(IGameContext gameContext, string loginName, byte? characterSlot = null)
    {
        ...
        try
        {
            ...
            var personality = BotPersonalityResolver.Resolve(character.Name);
    ...
    /// <summary>
    /// Stops and removes a specific bot identified by its account login name.
    /// Used by the admin dashboard kill action.
    /// </summary>
    /// <param name="loginName">The login name of the bot account to stop.</param>
    /// <returns><c>true</c> if the bot was found and stopped; <c>false</c> otherwise.</returns>
    public async ValueTask<bool> StopBotByLoginAsync(string loginName)
    {
        if (string.IsNullOrWhiteSpace(loginName))
        {
            return false;
        ...
        var key = this._bots.Keys.FirstOrDefault(k => k.StartsWith(loginName + "/", StringComparison.OrdinalIgnoreCase));
        ...
        {
            bot.Logger.LogWarning(ex, "Bot '{Key}' couldn't leave its party before admin kill.", key);
        }

        await StopAndDisposeAsync(bot, key, "admin kill").ConfigureAwait(false);
        return true;
    }
