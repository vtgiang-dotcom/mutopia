...
public sealed class BotPlayer : OfflinePlayer
{
    ...
    /// <inheritdoc />
    public override bool RespawnAndContinue => !PlugIns.ChatCommands.GuildWarEventChatCommandPlugIn.IsWarRunning;
    ...
    /// <summary>
    /// Gets the UTC time when this bot was spawned.
    /// </summary>
    public DateTime StartedAt { get; } = DateTime.UtcNow;

    /// <summary>
    /// Gets or sets the total experience gained since spawn.
    /// </summary>
    public long ExperienceGainedSinceStart { get; set; }

    /// <summary>
    /// Gets or sets the total zen earned since spawn.
    /// </summary>
    public long ZenEarnedSinceStart { get; set; }

    /// <summary>
    /// Gets or sets the total zen spent since spawn.
    /// </summary>
    public long ZenSpentSinceStart { get; set; }

    /// <summary>
    /// Gets or sets the total items picked up since spawn.
    /// </summary>
    public int ItemsPickedUpSinceStart { get; set; }

    /// <summary>
    /// Gets or sets the total items consumed since spawn.
    /// </summary>
    public int ItemsConsumedSinceStart { get; set; }

    /// <summary>
    /// Gets or sets the total items dropped since spawn.
    /// </summary>
    public int ItemsDroppedSinceStart { get; set; }

    /// <summary>
    /// Gets or sets the total jewels picked up since spawn.
    /// </summary>
    public int JewelsPickedUpSinceStart { get; set; }
    ...
    /// <inheritdoc />
    protected override async ValueTask AddExperienceCoreAsync(int experience, IAttackable? killedObject)
    {
        var prevExp = this.SelectedCharacter?.Experience ?? 0;
        await base.AddExperienceCoreAsync(experience, killedObject).ConfigureAwait(false);
        var currExp = this.SelectedCharacter?.Experience ?? 0;
        
        if (currExp > prevExp)
        {
            this.ExperienceGainedSinceStart += (currExp - prevExp);
        }
    }

    /// <inheritdoc />
    public override bool TryAddMoney(int value)
    {
        bool success = base.TryAddMoney(value);
        if (success && value > 0)
        {
            this.ZenEarnedSinceStart += value;
        }
        return success;
    }

    /// <inheritdoc />
    public override bool TryRemoveMoney(int value)
    {
        bool success = base.TryRemoveMoney(value);
        if (success && value > 0)
        {
            this.ZenSpentSinceStart += value;
        }
        return success;
    }

    /// <inheritdoc />
    public override async ValueTask OnPickedUpItemAsync(ILocateable item)
    {
        await base.OnPickedUpItemAsync(item).ConfigureAwait(false);
        
        if (item is DroppedItem droppedItem)
        {
            this.ItemsPickedUpSinceStart++;
            if (droppedItem.Item.Definition?.Group == 14)
            {
                this.JewelsPickedUpSinceStart++;
            }
        }
    }

    /// <inheritdoc />
    public override void OnItemConsumed(Item item)
    {
        base.OnItemConsumed(item);
        this.ItemsConsumedSinceStart++;
    }
