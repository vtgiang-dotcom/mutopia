...
void CCharMakeWin::Show(bool bShow)
{
    ...
    {
        CInput& rInput = CInput::Instance();
        SetPosition((rInput.GetScreenWidth() - 454) / 2, (rInput.GetScreenHeight() - 406) / 2);
...
{
    // Save the 2D (ortho) projection GlobalUBO holds on entry: the 3D block below overwrites it
    // with the perspective camera via BeginOpengl(), and EndOpengl() restores nothing (it is a no-op).
    // Without this restore, the sprite/button/text drawing that follows in RenderControls() runs
    // under the perspective matrix, which maps screen-space vertices out of the frustum and makes
    // the whole character-creation window (except the 3D preview) disappear.
    float pre3DProj[16];
    float pre3DView[16];
    memcpy(pre3DProj, GlobalUBO::Instance().GetProj(), sizeof(pre3DProj));
    memcpy(pre3DView, GlobalUBO::Instance().GetView(), sizeof(pre3DView));
    ...
    // Restore the 2D projection for the remaining UI draw calls of this window.
    GlobalUBO::Instance().SetProj(pre3DProj);
    GlobalUBO::Instance().SetView(pre3DView);
}