...
- **Git repo restored + repo hygiene pass (2026-08-27)**: Local `.git` had no `objects`/`refs` (never fully committed). Restored by copying objects/refs from a bare clone of `origin/main` (github.com/vtgiang-dotcom/mutopia), re-added remote, rebuilt index with `git add -A`. Found and reverted accidental deletions from an earlier restructure: `src/server-s16`, `src/client-s16`, `src/database/*.sql`, `.vscode/`, `GameLogic/PlugIns/BotAI/` had vanished from the working tree — all restored from HEAD. Excluded from commit: a misplaced Python venv under `src/server-s6/wwwroot/site-packages/`, Blazor RCL build output `wwwroot/_content/`, and CMake build dir `src/client-s6/build-x86/` — all three now gitignored. Committed+pushed as `b958f235`.

- **Critical Issues Fix Round 1 (2026-08-27)**: Fixed hardcoded DB passwords (moved to env vars) in web-portal/server-s6 configs, removed Docker port 8080 duplication, added Docker healthchecks (db/server/web), fixed AllowedHosts wildcards, created `.github/workflows/ci.yml` + `security.yml`, wrote `docs/DEPLOYMENT.md`. Also cleaned: removed obsolete backup `.csproj`, duplicate `appsettings_*.json`, replaced debug `Console.WriteLine` with `ILogger` in Dapr secret provider.
