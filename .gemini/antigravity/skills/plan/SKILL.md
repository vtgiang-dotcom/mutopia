...
- Your deliverable is a markdown plan saved inside the active workspace under `.kilo/plans/`.
...
- `.kilo/plans/YYYY-MM-DD_HHMMSS-<slug>.md`

Treat that as relative to the active working directory / backend workspace. Kilo file tools are backend-aware, so using this relative path keeps the plan with the workspace on local, docker, ssh, modal, and daytona backends.
...
See `testing-patterns` skill for details.
...
- Fresh `task` per task with full context
